...
I've written a pixel rotation shader that moves the screen by tiny amounts in set intervals (configurable). It's meant to be used with the CRT presets, to mitigate the uneven usage of pixels created by scanlines and the CRT mask when playing on an OLED display (it does NOT, however, mitigate uneven usage due to playing in 4:3, you'll just have to play stretched or use a border shader).
